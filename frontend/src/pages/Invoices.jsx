              ...
              <div key={tx.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-3">
                <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="font-bold text-slate-800 text-sm">{tx.id}</h3>
                      <p className="text-slate-500 text-xs">Ref: {tx.ref}</p>
                    </div>
                  </div>
                  <button className="p-2 text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors">
                    <Download size={18} />
                  </button>
                </div>
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-medium text-slate-800">{tx.date}</span>
                </div>
                <div className="flex justify-between items-center text-sm pt-2 border-t border-slate-50">
                  <span className="font-medium text-slate-500">Total:</span>
                  <span className="font-bold text-blue-700 text-base">Rp {tx.total.toLocaleString('id-ID')}</span>
                </div>
              </div>
            ))}
          </div>
